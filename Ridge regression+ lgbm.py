...
if __name__ == '__main__':
    ...
    ridge = Ridge(solver='auto', fit_intercept=True, alpha=0.4, max_iter=300, normalize=False, tol=0.001)
    ...
    develop=False
    # move the line below to where before "del sparse_merge"
    #mask = np.array(np.clip(sparse_merge.getnnz(axis=0) - 50, 0, 1), dtype=bool)
    # Remove features with document frequency <=100 for lgbm
    mask = np.array(np.clip(X_train.getnnz(axis=0) - 200, 0, 1), dtype=bool)
    ...
    if develop:
        train_X, valid_X, train_y, valid_y = train_test_split(X_train, y_train, test_size=0.05, random_state=100)
    else:
    ...
    model = lgb.train(params, train_set=d_train, num_boost_round=2500, valid_sets=watchlist, \
    ...
    #Ensemble
    submission.loc[:, 'price'] = np.expm1(0.7*predsR + 0.3*predsL)
    submission.loc[submission['price'] < 0.5, 'price'] = 0.5
